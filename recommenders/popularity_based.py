...
def get_top_kaggle_items(df, top_n=10):
    ...
    kaggle_item_popularity = df.groupby('StockCode')['Quantity'].sum().sort_values(ascending=False)
    #Get the top N items
    return kaggle_item_popularity.head(top_n)

#Function to get the top N items for fnb dataset
def get_top_fnb_items(df, top_n=10):
    """
    ...
    fnb_item_popularity = df_checkout['item'].value_counts().head(top_n)
    #Return the top N items
    return fnb_item_popularity